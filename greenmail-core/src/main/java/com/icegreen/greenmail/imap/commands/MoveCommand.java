...
class MoveCommand extends SelectedStateCommand implements UidEnabledCommand {
    ...
    public void doProcess(ImapRequestLineReader request,
        ...
        List<Long> movedUidsOld = new ArrayList<>();
        List<Long> movedUidsNew = new ArrayList<>();
        ...
        for (long uid : uids) {
            ...
            if (inSet) {
                long movedUid = currentMailbox.moveMessage(uid, toFolder);
                movedUidsOld.add(uid);
                movedUidsNew.add(movedUid);
            }
        }

        // response
        response.okResponse(generateCopyUidResponseCode(currentMailbox, movedUidsOld, movedUidsNew), "Moved UIDs.");
